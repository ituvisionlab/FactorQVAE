...
from quantizers.relaxed_vq_quantizer import RelaxedVQQuantizer
...
class FactorQVAE(Base):

    def __init__(self, config, img_dir):
        super(FactorQVAE, self).__init__(config, img_dir)
    ...
    def training_step(self, img, batch_idx):
        ...
        vae_loss = reconst_loss + aux_loss + vae_tc_loss
    ...
    def validation_step(self, img, batch_idx):
        ...
        vae_loss = reconst_loss + aux_loss + self.config.model.discriminator.gamma * vae_tc_loss
        ...
        self.log('test_additional_loss', aux_loss, prog_bar=True)
